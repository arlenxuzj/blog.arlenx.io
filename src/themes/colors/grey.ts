const hue = 223;
const saturation = 15;

const grey = {
  0: `hsl(${hue}, ${saturation}%, 100%)`,
  50: `hsl(${hue}, ${saturation}%, 95%)`,
  100: `hsl(${hue}, ${saturation}%, 90%)`,
  150: `hsl(${hue}, ${saturation}%, 85%)`,
  200: `hsl(${hue}, ${saturation}%, 80%)`,
  250: `hsl(${hue}, ${saturation}%, 75%)`,
  300: `hsl(${hue}, ${saturation}%, 70%)`,
  350: `hsl(${hue}, ${saturation}%, 65%)`,
  400: `hsl(${hue}, ${saturation}%, 60%)`,
  450: `hsl(${hue}, ${saturation}%, 55%)`,
  500: `hsl(${hue}, ${saturation}%, 50%)`,
  550: `hsl(${hue}, ${saturation}%, 45%)`,
  600: `hsl(${hue}, ${saturation}%, 40%)`,
  650: `hsl(${hue}, ${saturation}%, 35%)`,
  700: `hsl(${hue}, ${saturation}%, 30%)`,
  750: `hsl(${hue}, ${saturation}%, 25%)`,
  800: `hsl(${hue}, ${saturation}%, 20%)`,
  850: `hsl(${hue}, ${saturation}%, 15%)`,
  900: `hsl(${hue}, ${saturation}%, 10%)`,
  950: `hsl(${hue}, ${saturation}%, 5%)`
};

export default grey;
